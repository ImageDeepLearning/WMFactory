...
@dataclass
class OpenOasisGatewayRuntime:
    session_id: Optional[str] = None
    loaded: bool = False


class OpenOasisAdapter(WorldModelAdapter):
    model_id = "open-oasis"

    def __init__(self) -> None:
        configured_url = os.getenv("WM_OPENOASIS_URL", "http://127.0.0.1:9005").rstrip("/")
        parsed = urlparse(configured_url)

        self.runtime = OpenOasisGatewayRuntime()
        self.base_url = configured_url
        self.timeout = float(os.getenv("WM_MODEL_HTTP_TIMEOUT", "120"))
        self.start_timeout = float(os.getenv("WM_OPENOASIS_START_TIMEOUT", "1800"))
        self.autostart = os.getenv("WM_OPENOASIS_AUTOSTART", "1") == "1"
        self.startup_timeout = float(os.getenv("WM_OPENOASIS_STARTUP_TIMEOUT", "240"))
    ...
    def _log(self, message: str) -> None:
        print(f"[open-oasis] {message}", flush=True)
    ...
    def _ensure_service_up(self) -> None:
        ...
        if not self.autostart:
            raise RuntimeError(
                f"Open-Oasis service is not reachable at {self.base_url}. "
                "Start it manually or set WM_OPENOASIS_AUTOSTART=1."
            )
        ...
        if self._proc is not None and self._proc.poll() is None:
            ...
            configure_subprocess_cuda(env, "OPENOASIS", self._log)
            ...
            if self._log_fp is not None:
                self._log_fp.write(f"[open-oasis] spawned worker pid={self._proc.pid} cmd={' '.join(cmd)}\n")
                self._log_fp.flush()
        ...
        while time.time() < deadline:
            ...
            if self._proc is not None and self._proc.poll() is not None:
                raise RuntimeError(
                    f"Open-Oasis service exited early with code {self._proc.returncode}. "
                    f"Check log: {self.service_log}"
                )
            time.sleep(0.5)

        raise RuntimeError(
            f"Timed out waiting for open-oasis service startup at {self.base_url}. "
            f"Check log: {self.service_log}"
        )
    ...
    def _post(self, path: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        timeout = self.start_timeout if path in ("/sessions/start", "/sessions/reset", "/load") else self.timeout
        ...
        try:
            ...
            if path in ("/sessions/start", "/sessions/reset", "/load"):
                raise RuntimeError(f"Failed to connect to open-oasis service {url}: {exc}") from exc
            self._ensure_service_up()
            try:
                with httpx.Client(timeout=timeout) as client:
                    resp = client.post(url, json=payload)
            except Exception as exc2:
                raise RuntimeError(f"Failed to connect to open-oasis service {url}: {exc2}") from exc
        ...
        if resp.status_code >= 400:
            ...
            raise RuntimeError(f"Open-Oasis service error {resp.status_code}: {detail}")
        ...
        try:
            ...
            raise RuntimeError(f"Invalid JSON from open-oasis service {url}: {exc}") from exc
