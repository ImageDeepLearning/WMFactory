...
@dataclass
class LingBotWorldGatewayRuntime:
    session_id: Optional[str] = None
    loaded: bool = False


class LingBotWorldAdapter(WorldModelAdapter):
    model_id = "lingbot-world"

    def __init__(self) -> None:
        configured_url = os.getenv("WM_LINGBOTWORLD_URL", "http://127.0.0.1:9014").rstrip("/")
        parsed = urlparse(configured_url)

        self.runtime = LingBotWorldGatewayRuntime()
        self.base_url = configured_url
        self.timeout = float(os.getenv("WM_LINGBOTWORLD_HTTP_TIMEOUT", os.getenv("WM_MODEL_HTTP_TIMEOUT", "7200")))
        self.autostart = os.getenv("WM_LINGBOTWORLD_AUTOSTART", "1") == "1"
        self.startup_timeout = float(os.getenv("WM_LINGBOTWORLD_STARTUP_TIMEOUT", "3600"))

        self.service_host = os.getenv("WM_LINGBOTWORLD_HOST", parsed.hostname or "127.0.0.1")
        self.service_port = int(os.getenv("WM_LINGBOTWORLD_PORT", str(parsed.port or 9014)))
        self.service_dir = Path(os.getenv("WM_LINGBOTWORLD_SERVICE_DIR", str(ROOT / "services" / "lingbotworld")))
        self.service_python = os.getenv(
            "WM_LINGBOTWORLD_PYTHON",
            str(ROOT / "venvs" / "lingbot-world" / "bin" / "python"),
        )
        self.service_log = Path(
            os.getenv("WM_LINGBOTWORLD_LOG", str(self.service_dir / "lingbotworld_service.log"))
        )
        self.step_log_every = int(os.getenv("WM_LINGBOTWORLD_STEP_LOG_EVERY", "1"))
    ...
    def _log(self, message: str) -> None:
        print(f"[lingbot-world] {message}", flush=True)
    ...
    def _ensure_service_up(self) -> None:
        ...
        if not self.autostart:
            raise RuntimeError(
                f"LingBot-World service is not reachable at {self.base_url}. "
                "Start it manually or set WM_LINGBOTWORLD_AUTOSTART=1."
            )
        ...
        if self._proc is None or self._proc.poll() is not None:
            ...
            configure_subprocess_cuda(env, "LINGBOTWORLD", self._log)
            ...
            if self._log_fp is not None:
                self._log_fp.write(f"[lingbot-world] spawned worker pid={self._proc.pid} cmd={' '.join(cmd)}\n")
                self._log_fp.flush()
        ...
        while time.time() < deadline:
            ...
            if self._proc is not None and self._proc.poll() is not None:
                raise RuntimeError(
                    f"LingBot-World service exited early with code {self._proc.returncode}. "
                    f"Check log: {self.service_log}"
                )
            time.sleep(0.5)

        raise RuntimeError(
            f"Timed out waiting for LingBot-World service startup at {self.base_url}. "
            f"Check log: {self.service_log}"
        )
    ...
    def _post(self, path: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(f"Failed to connect to LingBot-World service {url}: {exc2}") from exc
        ...
        if resp.status_code >= 400:
            ...
            raise RuntimeError(f"LingBot-World service error {resp.status_code}: {detail}")

        try:
            data = resp.json()
        except Exception as exc:
            raise RuntimeError(f"Invalid JSON from LingBot-World service {url}: {exc}") from exc
        if should_log:
