...
class DiamondAdapter(WorldModelAdapter):
    ...
    def __init__(self) -> None:
        configured_url = os.getenv("WM_DIAMOND_URL", "http://127.0.0.1:9001").rstrip("/")
        parsed = urlparse(configured_url)

        self.runtime = DiamondGatewayRuntime()
        self.base_url = configured_url
        self.timeout = float(os.getenv("WM_MODEL_HTTP_TIMEOUT", "120"))
        self.autostart = os.getenv("WM_DIAMOND_AUTOSTART", "1") == "1"
        self.startup_timeout = float(os.getenv("WM_DIAMOND_STARTUP_TIMEOUT", "240"))

        self.service_host = os.getenv("WM_DIAMOND_HOST", parsed.hostname or "127.0.0.1")
        self.service_port = int(os.getenv("WM_DIAMOND_PORT", str(parsed.port or 9001)))
        self.service_dir = Path(os.getenv("WM_DIAMOND_SERVICE_DIR", str(ROOT / "services" / "diamond")))
        self.service_python = os.getenv("WM_DIAMOND_PYTHON", str(ROOT / "venvs" / "diamond" / "bin" / "python"))
        self.service_log = Path(os.getenv("WM_DIAMOND_LOG", str(self.service_dir / "diamond_service.log")))
        self.step_log_every = int(os.getenv("WM_DIAMOND_STEP_LOG_EVERY", "20"))
    ...
    def _log(self, message: str) -> None:
        print(f"[diamond] {message}", flush=True)
    ...
    def _ensure_service_up(self) -> None:
        ...
        if not self.autostart:
            raise RuntimeError(
                f"Diamond service is not reachable at {self.base_url}. "
                "Start it manually or set WM_DIAMOND_AUTOSTART=1."
            )

        if self._proc is not None and self._proc.poll() is None:
            # Process exists but health not yet ready.
            pass
        else:
            ...
            configure_subprocess_cuda(env, "DIAMOND", self._log)
            ...
            if self._log_fp is not None:
                self._log_fp.write(f"[diamond] spawned worker pid={self._proc.pid} cmd={' '.join(cmd)}\n")
                self._log_fp.flush()
        ...
        while time.time() < deadline:
            ...
            if self._proc is not None and self._proc.poll() is not None:
                raise RuntimeError(
                    f"Diamond service exited early with code {self._proc.returncode}. "
                    f"Check log: {self.service_log}"
                )
            time.sleep(0.5)

        raise RuntimeError(
            f"Timed out waiting for diamond service startup at {self.base_url}. "
            f"Check log: {self.service_log}"
        )
    ...
    def _post(self, path: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # One recovery attempt in case service crashed and restarts are enabled.
            self._ensure_service_up()
            try:
                with httpx.Client(timeout=self.timeout) as client:
                    resp = client.post(url, json=payload)
            except Exception as exc2:
                raise RuntimeError(f"Failed to connect to diamond service {url}: {exc2}") from exc
        ...
        if resp.status_code >= 400:
            ...
            raise RuntimeError(f"Diamond service error {resp.status_code}: {detail}")
        ...
        try:
            ...
            raise RuntimeError(f"Invalid JSON from diamond service {url}: {exc}") from exc
