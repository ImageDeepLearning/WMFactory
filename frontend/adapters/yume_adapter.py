...
@dataclass
class YumeGatewayRuntime:
    session_id: Optional[str] = None
    loaded: bool = False


class YumeAdapter(WorldModelAdapter):
    model_id = "yume"

    def __init__(self) -> None:
        configured_url = os.getenv("WM_YUME_URL", "http://127.0.0.1:9008").rstrip("/")
        parsed = urlparse(configured_url)

        self.runtime = YumeGatewayRuntime()
        self.base_url = configured_url
        self.timeout = float(os.getenv("WM_YUME_HTTP_TIMEOUT", os.getenv("WM_MODEL_HTTP_TIMEOUT", "900")))
        self.autostart = os.getenv("WM_YUME_AUTOSTART", "1") == "1"
        self.startup_timeout = float(os.getenv("WM_YUME_STARTUP_TIMEOUT", "300"))
    ...
    def _log(self, message: str) -> None:
        print(f"[yume] {message}", flush=True)
    ...
    def _ensure_service_up(self) -> None:
        ...
        if not self.autostart:
            raise RuntimeError(
                f"YUME service is not reachable at {self.base_url}. "
                "Start it manually or set WM_YUME_AUTOSTART=1."
            )
        ...
        if self._proc is not None and self._proc.poll() is None:
            ...
            configure_subprocess_cuda(env, "YUME", self._log)
            ...
            if self._log_fp is not None:
                self._log_fp.write(f"[yume] spawned worker pid={self._proc.pid} cmd={' '.join(cmd)}\n")
                self._log_fp.flush()
        ...
        while time.time() < deadline:
            ...
            if self._proc is not None and self._proc.poll() is not None:
                raise RuntimeError(
                    f"YUME service exited early with code {self._proc.returncode}. "
                    f"Check log: {self.service_log}"
                )
            time.sleep(0.5)

        raise RuntimeError(
            f"Timed out waiting for YUME service startup at {self.base_url}. "
            f"Check log: {self.service_log}"
        )
    ...
    def _post(self, path: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(f"Failed to connect to YUME service {url}: {exc2}") from exc
        ...
        if resp.status_code >= 400:
            ...
            raise RuntimeError(f"YUME service error {resp.status_code}: {detail}")
        ...
        try:
            ...
            raise RuntimeError(f"Invalid JSON from YUME service {url}: {exc}") from exc
