...
@dataclass
class WorldPlayGatewayRuntime:
    session_id: Optional[str] = None
    loaded: bool = False


class WorldPlayAdapter(WorldModelAdapter):
    model_id = "worldplay"

    def __init__(self) -> None:
        configured_url = os.getenv("WM_WORLDPLAY_URL", "http://127.0.0.1:9009").rstrip("/")
        parsed = urlparse(configured_url)

        self.runtime = WorldPlayGatewayRuntime()
        self.base_url = configured_url
        self.timeout = float(os.getenv("WM_WORLDPLAY_HTTP_TIMEOUT", os.getenv("WM_MODEL_HTTP_TIMEOUT", "1800")))
        self.autostart = os.getenv("WM_WORLDPLAY_AUTOSTART", "1") == "1"
        self.startup_timeout = float(os.getenv("WM_WORLDPLAY_STARTUP_TIMEOUT", "300"))
    ...
    def _log(self, message: str) -> None:
        print(f"[worldplay] {message}", flush=True)

    def _tail_service_log(self, lines: int = 60) -> str:
        try:
    ...
    def _ensure_service_up(self) -> None:
        ...
        if not self.autostart:
            raise RuntimeError(
                f"WorldPlay service is not reachable at {self.base_url}. "
                "Start it manually or set WM_WORLDPLAY_AUTOSTART=1."
            )
        ...
        if self._proc is None or self._proc.poll() is not None:
            ...
            worldplay_root = ROOT / "models" / "HY-WorldPlay"
            wan_root = worldplay_root / "wan"
            existing_pythonpath = env.get("PYTHONPATH", "")
            pythonpath_entries = [str(worldplay_root), str(wan_root)]
            if existing_pythonpath:
                pythonpath_entries.append(existing_pythonpath)
            env["PYTHONPATH"] = os.pathsep.join(pythonpath_entries)
            env.setdefault("WM_WORLDPLAY_ROOT", str(worldplay_root))
            if os.getenv("WM_WORLDPLAY_CUDA_VISIBLE_DEVICES") is not None:
                env["CUDA_VISIBLE_DEVICES"] = os.getenv("WM_WORLDPLAY_CUDA_VISIBLE_DEVICES", "")
            ...
            t.start()
            if self._log_fp is not None:
                self._log_fp.write(f"[worldplay] spawned worker pid={self._proc.pid} cmd={' '.join(cmd)}\n")
                self._log_fp.flush()
        ...
        while time.time() < deadline:
            ...
            if self._proc is not None and self._proc.poll() is not None:
                raise RuntimeError(
                    f"WorldPlay service exited early with code {self._proc.returncode}. "
                    f"Check log: {self.service_log}"
                )
            time.sleep(0.5)

        raise RuntimeError(
            f"Timed out waiting for WorldPlay service startup at {self.base_url}. "
            f"Check log: {self.service_log}"
        )
    ...
    def _post(self, path: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(f"Failed to connect to WorldPlay service {url}: {exc2}") from exc
        ...
        if resp.status_code >= 400:
            ...
            raise RuntimeError(f"WorldPlay service error {resp.status_code}: {detail}")
        ...
        try:
            ...
            raise RuntimeError(f"Invalid JSON from WorldPlay service {url}: {exc}") from exc
