...
@dataclass
class WhamGatewayRuntime:
    session_id: Optional[str] = None
    loaded: bool = False


class WhamAdapter(WorldModelAdapter):
    model_id = "wham"

    def __init__(self) -> None:
        configured_url = os.getenv("WM_WHAM_URL", "http://127.0.0.1:9007").rstrip("/")
        parsed = urlparse(configured_url)

        self.runtime = WhamGatewayRuntime()
        self.base_url = configured_url
        self.timeout = float(os.getenv("WM_MODEL_HTTP_TIMEOUT", "120"))
        self.start_timeout = float(os.getenv("WM_WHAM_START_TIMEOUT", "300"))
        self.autostart = os.getenv("WM_WHAM_AUTOSTART", "1") == "1"
        self.startup_timeout = float(os.getenv("WM_WHAM_STARTUP_TIMEOUT", "180"))

        self.service_host = os.getenv("WM_WHAM_HOST", parsed.hostname or "127.0.0.1")
        self.service_port = int(os.getenv("WM_WHAM_PORT", str(parsed.port or 9005)))
        self.service_dir = Path(os.getenv("WM_WHAM_SERVICE_DIR", str(ROOT / "services" / "wham")))
        self.service_python = os.getenv("WM_WHAM_PYTHON", str(ROOT / "venvs" / "wham" / "bin" / "python"))
        self.service_log = Path(os.getenv("WM_WHAM_LOG", str(self.service_dir / "wham_service.log")))
    ...
    def _log(self, message: str) -> None:
        print(f"[wham] {message}", flush=True)
    ...
    def _ensure_service_up(self) -> None:
        ...
        if not self.autostart:
            raise RuntimeError(
                f"WHAM service is not reachable at {self.base_url}. "
                "Start it manually or set WM_WHAM_AUTOSTART=1."
            )
        ...
        if self._proc is not None and self._proc.poll() is None:
            ...
            configure_subprocess_cuda(env, "WHAM", self._log)
            ...
            if self._log_fp is not None:
                self._log_fp.write(f"[wham] spawned worker pid={self._proc.pid} cmd={' '.join(cmd)}\n")
                self._log_fp.flush()
        ...
        while time.time() < deadline:
            ...
            if self._proc is not None and self._proc.poll() is not None:
                raise RuntimeError(
                    f"WHAM service exited early with code {self._proc.returncode}. "
                    f"Check log: {self.service_log}"
                )
            time.sleep(0.5)

        raise RuntimeError(
            f"Timed out waiting for wham service startup at {self.base_url}. "
            f"Check log: {self.service_log}"
        )
    ...
    def _post(self, path: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if path in ("/sessions/start", "/sessions/reset"):
                raise RuntimeError(f"Failed to connect to wham service {url}: {exc}") from exc
            self._ensure_service_up()
            try:
                with httpx.Client(timeout=timeout) as client:
                    resp = client.post(url, json=payload)
            except Exception as exc2:
                raise RuntimeError(f"Failed to connect to wham service {url}: {exc2}") from exc
        ...
        if resp.status_code >= 400:
            ...
            raise RuntimeError(f"WHAM service error {resp.status_code}: {detail}")
        ...
        try:
            ...
            raise RuntimeError(f"Invalid JSON from wham service {url}: {exc}") from exc
