...
class RFlowScheduler:
    ...
    def sample(
        ...
        if context_parallel_util.get_cp_rank() == 0:
            ...
            # text encoder can run on CPU; move embeddings to current inference device.
            model_args["y"] = model_args["y"].to(device, non_blocking=True)
            model_args["y_mask"] = model_args["y_mask"].to(device, non_blocking=True)
            if context_parallel_util.get_cp_size() > 1:
