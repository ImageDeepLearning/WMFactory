...
@dataclass
class MineWorldGatewayRuntime:
    session_id: Optional[str] = None
    loaded: bool = False


class MineWorldAdapter(WorldModelAdapter):
    model_id = "mineworld"
    ...
    def __init__(self) -> None:
        ...
        self.autostart = os.getenv("WM_MINEWORLD_AUTOSTART", "1") == "1"
    ...
    def _log(self, message: str) -> None:
        print(f"[mineworld] {message}", flush=True)
    ...
    def _ensure_service_up(self) -> None:
        ...
        if not self.autostart:
            raise RuntimeError(
                f"MineWorld service is not reachable at {self.base_url}. "
                "Start it manually or set WM_MINEWORLD_AUTOSTART=1."
            )
        ...
        if self._proc is not None and self._proc.poll() is None:
            ...
            env.setdefault("WM_MINEWORLD_CKPT", str(ROOT / "mnt" / "mineworld" / "checkpoints" / "1200M_32f.ckpt"))
            env.setdefault("WM_MINEWORLD_CONFIG", str(ROOT / "models" / "mineworld" / "configs" / "1200M_32f.yaml"))
            configure_subprocess_cuda(env, "MINEWORLD", self._log)
            ...
            if self._log_fp is not None:
                self._log_fp.write(f"[mineworld] spawned worker pid={self._proc.pid} cmd={' '.join(cmd)}\n")
                self._log_fp.flush()
        ...
        while time.time() < deadline:
            ...
            if self._proc is not None and self._proc.poll() is not None:
                raise RuntimeError(
                    f"MineWorld service exited early with code {self._proc.returncode}. "
                    f"Check log: {self.service_log}"
                )
            time.sleep(0.5)

        raise RuntimeError(
            f"Timed out waiting for mineworld service startup at {self.base_url}. "
            f"Check log: {self.service_log}"
        )
    ...
    def _post(self, path: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(f"Failed to connect to mineworld service {url}: {exc2}") from exc
        ...
        if resp.status_code >= 400:
            ...
            raise RuntimeError(f"MineWorld service error {resp.status_code}: {detail}")
        ...
        try:
            ...
            raise RuntimeError(f"Invalid JSON from mineworld service {url}: {exc}") from exc
