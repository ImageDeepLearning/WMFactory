...
@dataclass
class WorldFMGatewayRuntime:
    session_id: Optional[str] = None
    loaded: bool = False


class WorldFMAdapter(WorldModelAdapter):
    model_id = "worldfm"

    def __init__(self) -> None:
        configured_url = os.getenv("WM_WORLDFM_URL", "http://127.0.0.1:9002").rstrip("/")
        parsed = urlparse(configured_url)

        self.runtime = WorldFMGatewayRuntime()
        self.base_url = configured_url
        self.timeout = float(os.getenv("WM_MODEL_HTTP_TIMEOUT", "120"))
        self.start_timeout = float(os.getenv("WM_WORLDFM_START_TIMEOUT", "7200"))
        self.autostart = os.getenv("WM_WORLDFM_AUTOSTART", "1") == "1"
        self.startup_timeout = float(os.getenv("WM_WORLDFM_STARTUP_TIMEOUT", "240"))
    ...
    def _log(self, message: str) -> None:
        print(f"[worldfm] {message}", flush=True)
    ...
    def _ensure_service_up(self) -> None:
        ...
        if not self.autostart:
            raise RuntimeError(
                f"WorldFM service is not reachable at {self.base_url}. "
                "Start it manually or set WM_WORLDFM_AUTOSTART=1."
            )
        ...
        if self._proc is not None and self._proc.poll() is None:
            ...
            configure_subprocess_cuda(env, "WORLDFM", self._log)
            ...
            if self._log_fp is not None:
                self._log_fp.write(f"[worldfm] spawned worker pid={self._proc.pid} cmd={' '.join(cmd)}\n")
                self._log_fp.flush()
        ...
        while time.time() < deadline:
            ...
            if self._proc is not None and self._proc.poll() is not None:
                raise RuntimeError(
                    f"WorldFM service exited early with code {self._proc.returncode}. "
                    f"Check log: {self.service_log}"
                )
            time.sleep(0.5)

        raise RuntimeError(
            f"Timed out waiting for worldfm service startup at {self.base_url}. "
            f"Check log: {self.service_log}"
        )
    ...
    def _post(self, path: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # start/reset are non-idempotent and can be very long-running: never retry.
            if path in ("/sessions/start", "/sessions/reset"):
                raise RuntimeError(f"Failed to connect to worldfm service {url}: {exc}") from exc
            self._ensure_service_up()
            try:
                with httpx.Client(timeout=timeout) as client:
                    resp = client.post(url, json=payload)
            except Exception as exc2:
                raise RuntimeError(f"Failed to connect to worldfm service {url}: {exc2}") from exc
        ...
        if resp.status_code >= 400:
            ...
            raise RuntimeError(f"WorldFM service error {resp.status_code}: {detail}")
        ...
        try:
            ...
            raise RuntimeError(f"Invalid JSON from worldfm service {url}: {exc}") from exc
