...
@dataclass
class GameCraftGatewayRuntime:
    session_id: Optional[str] = None
    loaded: bool = False


class GameCraftAdapter(WorldModelAdapter):
    model_id = "gamecraft"

    def __init__(self) -> None:
        configured_url = os.getenv("WM_GAMECRAFT_URL", "http://127.0.0.1:9012").rstrip("/")
        parsed = urlparse(configured_url)

        self.runtime = GameCraftGatewayRuntime()
        self.base_url = configured_url
        self.timeout = float(os.getenv("WM_GAMECRAFT_HTTP_TIMEOUT", os.getenv("WM_MODEL_HTTP_TIMEOUT", "3600")))
        self.autostart = os.getenv("WM_GAMECRAFT_AUTOSTART", "1") == "1"
        self.startup_timeout = float(os.getenv("WM_GAMECRAFT_STARTUP_TIMEOUT", "1800"))

        self.service_host = os.getenv("WM_GAMECRAFT_HOST", parsed.hostname or "127.0.0.1")
        self.service_port = int(os.getenv("WM_GAMECRAFT_PORT", str(parsed.port or 9012)))
        self.service_dir = Path(os.getenv("WM_GAMECRAFT_SERVICE_DIR", str(ROOT / "services" / "gamecraft")))
        self.service_python = os.getenv(
            "WM_GAMECRAFT_PYTHON",
            str(ROOT / "venvs" / "Hunyuan-GameCraft-1.0" / "bin" / "python"),
        )
        self.service_log = Path(os.getenv("WM_GAMECRAFT_LOG", str(self.service_dir / "gamecraft_service.log")))
        self.step_log_every = int(os.getenv("WM_GAMECRAFT_STEP_LOG_EVERY", "1"))
    ...
    def _pick_gpu(self) -> str:
        configured = os.getenv("WM_GAMECRAFT_CUDA_VISIBLE_DEVICES")
        ...
        try:
            ...
            for line in output.strip().splitlines():
                ...
                candidates.append((int(mem_str), idx_str))
            candidates.sort()
    ...
    def _log(self, message: str) -> None:
        print(f"[gamecraft] {message}", flush=True)
    ...
    def _ensure_service_up(self) -> None:
        ...
        if not self.autostart:
            raise RuntimeError(
                f"GameCraft service is not reachable at {self.base_url}. "
                "Start it manually or set WM_GAMECRAFT_AUTOSTART=1."
            )
        ...
        if self._proc is None or self._proc.poll() is not None:
            ...
            env.setdefault("WM_GAMECRAFT_CUDA_VISIBLE_DEVICES", self._pick_gpu())
            ...
            if self._log_fp is not None:
                self._log_fp.write(f"[gamecraft] spawned worker pid={self._proc.pid} cmd={' '.join(cmd)}\n")
                self._log_fp.flush()
        ...
        while time.time() < deadline:
            ...
            if self._proc is not None and self._proc.poll() is not None:
                raise RuntimeError(
                    f"GameCraft service exited early with code {self._proc.returncode}. "
                    f"Check log: {self.service_log}"
                )
            time.sleep(0.5)

        raise RuntimeError(
            f"Timed out waiting for GameCraft service startup at {self.base_url}. "
            f"Check log: {self.service_log}"
        )
    ...
    def _post(self, path: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(f"Failed to connect to GameCraft service {url}: {exc2}") from exc
        ...
        if resp.status_code >= 400:
            ...
            raise RuntimeError(f"GameCraft service error {resp.status_code}: {detail}")
